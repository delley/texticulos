# Links Interessantes
Apenas alguns links interessantes.

## Repositórios do Git

* [Rafael 'Vicio' Augusto](https://github.com/rafaelvicio)

## Python

* [Desenvolvendo chatbots para o Facebook Messenger com Python e Django 1/3](https://medium.com/botsbrasil/desenvolvendo-chatbots-para-o-facebook-messenger-com-python-e-django-1-3-eb9e6a5eaff2)

## Java

### Spring

* [CONFIGURING SPRING BOOT FOR POSTGRESQL](https://springframework.guru/configuring-spring-boot-for-postgresql/)
* [SPRING BOOT WEB APPLICATION – PART 3 – SPRING DATA JPA](https://springframework.guru/spring-boot-web-application-part-3-spring-data-jpa/)
* [REST Server with Spring Data, Spring Boot and PostgreSQL](http://mrbool.com/rest-server-with-spring-data-spring-boot-and-postgresql/34023)
* [Spring Data JPA : Custom Repository / Query Methods Example](http://javabeat.net/spring-data-custom-repository/)
* [Domine o Spring](https://domineospring.wordpress.com/)

### Spark Framework

* [Spark Tutoriais](https://sparktutorials.github.io/)
* [Blog Caelum - Introdução aos microservices com Spark: retorno de dados dos correios](http://blog.caelum.com.br/introducao-aos-microservices-com-spark-retorno-de-dados-dos-correios/)

### Wildfly

* [Using try-with-resources to close database connections](http://www.mastertheboss.com/jboss-server/jboss-datasource/using-try-with-resources-to-close-database-connections)

### MongoDB

* [Java Connecting to MongoDB 3.2 Examples](https://avaldes.com/java-connecting-to-mongodb-3-2-examples/)
* [MongoDB 3 With Java](http://www.javaroots.com/2015/05/mongodb-3-with-java.html)
* [Use a single MongoClient across a JavaEE web service](http://stackoverflow.com/questions/26084965/use-a-single-mongoclient-across-a-javaee-web-service)
* [MongoDB Java Servlet Web Application Example Tutorial](http://www.journaldev.com/4011/mongodb-java-servlet-web-application-example-tutorial)
* [Developing Single Page Web Applications using Java 8, Spark, MongoDB, and AngularJS](https://blog.openshift.com/developing-single-page-web-applications-using-java-8-spark-mongodb-and-angularjs/)
* [shekhargulati/todoapp-spark](https://github.com/shekhargulati/todoapp-spark)
* [CRUD operations in MongoDB using Java](http://blog.bimarian.com/crud-operations-in-mongodb-using-java/)
* [MongoDB Java Driver 3.0 Quick reference](http://www.mastertheintegration.com/nosql-databases/mongodb/mongodb-java-driver-3-0-quick-reference.html)
* [Java Code Examples for com.mongodb.client.model.Filters](http://www.programcreek.com/java-api-examples/index.php?api=com.mongodb.client.model.Filters)
* [MongoDB Java Driver 3.0 - Filters](http://mongodb.github.io/mongo-java-driver/3.0/builders/filters/)
* [MongoDB Java](https://mongodbwise.wordpress.com/2014/05/20/mongodb-java/)
* [NoSQL com MongoDB e Persistência em Java](http://www.mballem.com/post/nosql-com-mongodb-e-persistencia-em-java/)
